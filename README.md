# 轨道交通/地铁PIS车内导视系统

<div align="center">

[![License: CC BY-NC-SA 4.0](https://img.shields.io/badge/License-CC%20BY--NC--SA%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by-nc-sa/4.0/) ![Maintenance](https://img.shields.io/badge/Maintenance-Solo-blue)

[简体中文] | [[English](./README_en.md)]

</div>

## 声明

### 简要

允许看、允许改、允许个人玩，但禁止赚钱，禁止闭源

### 正式

本项目源代码完全公开，但并非“公共领域”资产。项目采用 **CC BY-NC-SA 4.0** 协议：
1.  **非商用：** 您不可将本项目用于任何商业盈利场景。
2.  **禁止闭源：** 任何基于本项目的修改版本，必须保持开源并沿用相同协议。
3.  **署名：** 转载或引用必须保留原作者署名。
### 维护模式
这是一个个人学习的项目。为了保持代码库与设计方向的统一：
*   ❌ 目前**不接受任何 Pull Request (PR)。**
*   如果您有好的想法，欢迎**直接**在 Issues 中讨论以便促进更新

### 镜像地址
国内镜像地址(每8小时同步): [gitea镜像](http://8.130.22.186:53000/DPCau/Open-Metro-PIS)

### 快速体验
> 建议先阅读[按键使用](#use-by-keyboard)说明，再进行体验
> 建议使用电脑端浏览器访问，移动端浏览器也进行了按钮适配，若宽度不够可以进行缩放
> 本在线部署为防盗使用水印，但若自行手动部署**默认关闭**，可在`global_config.json`中设置

[点击这里](http://8.130.22.186:58089/) 即可体验本项目的功能

## 更新说明

- 2026-06-16 新增非环线正弦波布局模式（P键），原自动模式移至[键，新增触控按钮隐藏快捷键（M键跨页面保持），界面2换行处改为SVG圆弧过渡
- 2026-02-22 调整详情页布局，修复其它问题
- 2026-02-07 修改若干显示问题，新增route的disable禁用，更新线路
- 2026-01-09 修复一些线路信息，同时新增高级配置，新增帧率和窗口大小配置项

<details>
<summary>点击展开/收起更新历史</summary>

- 2026-01-02 新增了线路预览的detail模式（用于显示环线、直达等特殊情况），增加部分回库路线
- 2025-12-31 修复详情页环线不连续问题，以及在站点数目不足时用经过站填充
- 2025-12-28 修复CRT直快列车问题，新增4号线回库线路
- 2025-12-26 新增蓉2号线内容，完善支线判定
- 2025-12-25 新增到达站页，绑定新按键操作方式，修复环线部分问题，增加到站页，增加详情页新展示模式，新增部署水印用于演示防盗，适配手机端
- 2025-12-24 新增按键操作方式，与整体主题色同步
- 2025-12-23 修复岔路反向问题，新增环线的two_line模式
- 2025-12-21 由于2025年12月20日至2026年10月成都2号线大面铺至龙泉驿停运，目前删除路线，同时替换README中交路演示为4号线
- 2025-12-11 完成了单行(包括岔路)，双行与自动模式，更新了文档展示图
- 2025-12-10 修复了环线下一站为终点站时被判断为起点的bug
- 2025-12-09 修复了详情页环线不连续的问题，环线线路终点站仅以terminal_station为准
- 2025-12-07 更新13、30号线内容，暂未更新2号线新站

</details>


## 项目介绍

本项目基于前后端地铁数据实现一个车内乘客信息系统(PIS)，提供实时的线路信息、到站提醒、站点导航等功能。


## 功能特性

1. **下一站信息显示**：展示当前线路、下一站信息、开门方向等
2. **线路图显示**：展示完整线路图和当前位置
3. **站点详情显示**：展示站点的换乘信息
4. **到达站x显示**：展示到达目标站点、开门方向等信息

## 效果展示

> 以下展示图为2025年12月25日更新的效果

1. 运营线路与下一站显示

- 普通线路
  - 正向
    
    万盛往西河
    
    ![image-20251225131724792](./assets/image-20251225131724792.png)
    
  - 反向
  
    西河往万盛
    
    ![image-20251225131734954](./assets/image-20251225131734954.png)


  - 小交路

    光华公园往来龙
    
    ![image-20251225131748340](./assets/image-20251225131748340.png)


- 环线线路

  - 外环
  
    - 普通模式
  
    ![image-20251225131806295](./assets/image-20251225131806295.png)
  
    - 详细模式

    ![image-20260102202741392](./assets/image-20260102202741392.png)
  
  - 内环
  
    - 普通模式
  
    ![image-20251225131818098](./assets/image-20251225131818098.png)
  
    - 详细模式

    ![image-20260102202758807](./assets/image-20260102202758807.png)
  
  - 有终点站
  
    往崔家店
    
    - 普通模式

    ![image-20251225131833878](./assets/image-20251225131833878.png)

    - 详细模式
    
    ![image-20260102202836162](./assets/image-20260102202836162.png)

- 含直达的线路

![image-20260102203016708](./assets/image-20260102203016708.png)

![image-20260102203029812](./assets/image-20260102203029812.png)


2. 线路图显示

- 普通线路
  - 单行模式 one_line
    > 单行模式下所有站在同一行显示不会换行，适合站数量较少的线路
    > 同时**只有**单行模式支持**分岔路**的显示
    - 普通
      - 大交路

        九江北往高洪

      ![image-20251225131922596](./assets/image-20251225131922596.png)
      
      - 小交路
      
        九江北往机车厂

      ![image-20251225131952619](./assets/image-20251225131952619.png)

    - 分岔路

      > 分叉路在线路配置上与普通交路大致一样，只需单独增加`branch:继承的目标route`字段即可

      - 主路
      韦家碾往科学城
    
      ![image-20251225132057046](./assets/image-20251225132057046.png)

      - 岔路  
      ​        韦家碾往五根松

      ![image-20251225132109980](./assets/image-20251225132109980.png)


  - 双行模式 two_line
    > 双行模式下所有站平均会在两行显示，适合站数量中等的线路
    > 该模式下**不支持**岔路显示，使用后只会显示主路与岔路其中一条
    - 大交路

      万盛往西河
      
    
    ![image-20251225132738261](./assets/image-20251225132738261.png)
    
    - 小交路
    
      光华公园往来龙
    
    ![image-20251225132749516](./assets/image-20251225132749516.png)    
    
  - 自动模式 auto
    > 自动模式下会自动换行，适合站数量较多的线路
    > 该模式下**不支持**岔路显示，使用后只会显示主路与岔路其中一条
    > 自动模式下支持使用字段`station_spacing_multiplier`来调整站间距倍数(>1)，默认值为1.0
    - 大交路

      望丛祠往兰家沟
    
    ![image-20251225132855616](./assets/image-20251225132855616.png)
    
    - 小交路
    
      望丛祠往沈阳路
      
    
    ![image-20251225132904965](./assets/image-20251225132904965.png)
    
    
    
- 环线线路
  - 双行模式 two_line
  
    > 环线双行模式下站点均匀分布在上下两侧
  
    - 全程
    
    ![image-20251225133003841](./assets/image-20251225133003841.png)
  
  
    - 有终点站
      往崔家店
  
    ![image-20251225133015534](./assets/image-20251225133015534.png)
      
  - 自动模式 auto
  
    > 自动模式下会均匀分布在环线的四周
  
    - 全程
    
    ![image-20251225133057836](./assets/image-20251225133057836.png)

    - 有终点站
  
      往崔家店
  
    ![image-20251225133042485](./assets/image-20251225133042485.png)
  
  
  3. 站点详情显示

    - 默认模式
    
      - 起点
      
        > 起点瓦窑滩左侧不会显示有站点

      ![image-20251225133133985](./assets/image-20251225133133985.png)
    
      - 中间
      
        > 两侧均有站点

      ![image-20251225133201280](./assets/image-20251225133201280.png)

      - 终点
      
        > 终点站龙安右侧不会显示有站点
      
      
      ![image-20251225133221177](./assets/image-20251225133221177.png)
    
    - 列表模式
    
      - 起点
      
        > 起点万盛上方不会显示有站点
  
      ![image-20251225133339346](./assets/image-20251225133339346.png)
    
      - 中间
      
        > 两侧均有站点
  
      ![image-20251225133354043](./assets/image-20251225133354043.png)
  
      - 终点
      
        > 终点站西河下方不会显示有站点
      
      
      ![image-20251225133414352](./assets/image-20251225133414352.png)
  
  4. 到达站页
    > 到达站页展示当前线路的到达站、开门方向等信息
  
    - 6节编组
  
    ![image-20251225133557867](./assets/image-20251225133557867.png)
  
    - 8节编组
  
    ![image-20251225133528089](./assets/image-20251225133528089.png)
  
    - 4节编组

    ![image-20251225133646249](./assets/image-20251225133646249.png)    

## 使用方法

0. 字体下载与名称、站台配置:

- 下载字体文件 [font.ttf](https://wwyq.lanzouo.com/inhZH39s00vc)
- 将字体文件放入`static/fonts/`目录下
- 在data目录下建立你的城市文件夹，示例已经给出了成都地铁线路配置`data/chengdu/`与重庆轨道交通线路配置`data/chongqing/`
- 在项目根目录下`city_config.json`配置你的城市文件夹，示例已经绑定了了成都地铁配置

1. 安装依赖(只需首次使用)：
```bash
pip install -r requirements.txt
```

2. 运行应用：
```bash
python app.py
```

3. 访问系统：
在浏览器中访问 http://localhost:8089

4. 切换站点、线路

<a id="use-by-keyboard"></a>
**a. 通过按键(推荐)**
- 使用数字键`1`,`2`,`3`,`4`分别切换到运营线路页，线路地图页，线路详情页和到达站页
- 使用`A`与`D`键切换上下站
- 使用`W`与`S`键切换交路
- 使用`F`键切换到下一站但不更新页面(使得下一次切换页面显示的是下个站点)
- 使用`R`键切换方向
- 使用`T`键切换开门方向
- 使用`M`键切换触控按钮显示/隐藏
- 在线路预览页(对应数字键`1`)，使用`I`,`O`分别切换默认模式与详细模式
- 在线路地图页(对应数字键`2`)，使用`I`,`O`,`[`分别切换单行模式、双行模式与自动模式（环线下按`I`也切换到双行），`P`键切换正弦波模式（仅非环线）
- 在线路详情页(对应数字键`3`)，使用`I`,`O`分别切换默认模式与列模式
- 使用`K`,`L`键切换运营线路

b. 在`current_state.json`下热修改站点信息
对`current_state.json`下的`current_state`变量进行修改
```json
{
    "line_name": "line_1",
    "route_name": "route1",
    "next_station": "韦家碾",
    "direction": 0,
    "door_side": "本侧",
    "current_carriage": 3
}
```
保存后自动动态加载上面的配置，刷新浏览器生效(若修改不符合语法可能导致程序停止，重新执行步骤2即可)

> 注意！若在启动过程中修改了`data/`下的文件，需要终端按下`Ctrl+C`中断原程序，重新执行步骤2才能生效

## 数据说明
- **global_config.json**

决定全局配置，例如是否开启水印、是否开启提示等

>advance_settings:高级设置
>>enable_advance_settings:是否开启高级设置
>>open_in_windows:是否在窗口中打开，打开后启动程序打开窗口
>>windows_size:窗口大小，例如`1470x720`，为`full`时全屏
>>windows_fps:窗口刷新频率，为-1时不限制
>>expose_to_network:是否暴露到网络
>>network_port:网络端口，默认8089

- **city_config.json**

决定展示哪个城市的地铁线路，示例已经绑定了成都地铁

- **config.json**

决定标签页显示的名称、年份等信息

- **route.json**：包含各线路的名称、类型和服务信息

字段解释
- line_name: 线路名称
- run_style: 运营路线显示样式，default为默认模式，detail为详细模式
- type: 线路类型，linear为普通线路，loop为环线线路
- layout: 线路布局，auto为自动布局，two_line为双线路布局，one_line为单线路布局，sine为正弦波布局（仅非环线），**环线线路不支持one_line布局**
- detail_style: 线路详情显示样式，default为默认模式，column为列模式
- carriage_count: 车厢编组数目，决定到达站显示车厢数
- services
  - type: 交路
  - stations: 站点列表，按顺序排列
  - label: 交路名称，用于显示在运营路线上
  - terminal_station: 终点站，仅环线线路需要配置
  - branch: 继承自哪里的分叉
  - group: 所属组，决定运营路线显示组

普通线路以成都地铁17号线为例：
```json
"line_17": {
  "line_name": "17号线-Line 17",
  "type": "linear",
  "layout": "one_line",
  "detail_style": "default",
  "carriage_count": 8,
  "station_spacing_multiplier": 1,
  "services": [
    {
      "service_name": "route1",
      "stations": [
        "九江北", "白佛桥", "机投桥", "阳公桥", "龙爪堰", "清水河大桥", "浣花里", "省骨科医院", "小南街", "人民公园", "西大街", "城隍庙", "红星桥", "建设北路", "踏水桥", "二仙桥", "机车厂", "人民塘", "航天路", "威灵", "高洪"
      ]
    },
    {
      "service_name": "route2",
      "stations": [
        "九江北", "白佛桥", "机投桥", "阳公桥", "龙爪堰", "清水河大桥", "浣花里", "省骨科医院", "小南街", "人民公园", "西大街", "城隍庙", "红星桥", "建设北路", "踏水桥", "二仙桥", "机车厂"
      ]
    }
  ]
}
```

普通线路**分叉**以成都地铁1号线为例：
> 注意route2是继承route1的分叉

```json
"line_1": {
  "line_name": "1号线",
  "type": "linear",
  "layout": "auto",
  "detail_style": "default",
  "carriage_count": 6,
  "services": [
    {
      "type": "route1",
      "stations": [
        "韦家碾", "升仙湖", "火车北站", "人民北路", "文殊院", "骡马市", "天府广场", "锦江宾馆", "华西坝", "省体育馆", "倪家桥", "桐梓林", "火车南站", "高新", "金融城", "孵化园", "锦城广场", "世纪城", "天府三街", "天府五街", "华府大道", "四河", "华阳", "海昌路", "广福", "红石公园", "麓湖", "武汉路", "天府公园", "西博城", "广州路", "兴隆湖", "科学城"
      ]
    },
    {
      "type": "route2",
      "branch": "route1",
      "stations": [
        "韦家碾", "升仙湖", "火车北站", "人民北路", "文殊院", "骡马市", "天府广场", "锦江宾馆", "华西坝", "省体育馆", "倪家桥", "桐梓林", "火车南站", "高新", "金融城", "孵化园", "锦城广场", "世纪城", "天府三街", "天府五街", "华府大道", "四河", "广都", "五根松"
      ]
    }
  ]
}
```

环线线路以成都地铁7号线为例：
```json
"line_7": {
  "line_name": "7号线",
  "type": "loop",
  "run_style": "detail",
  "layout": "auto",
  "detail_style": "default",
  "carriage_count": 6,
  "services": [
    {
      "service_name": "route1",
      "terminal_station": "",
      "stations": [
        "火车北站", "北站西二路", "九里堤", "西南交大", "花照壁", "茶店子", "一品天下", "金沙博物馆", "文化宫", "东坡路", "龙爪堰", "武侯大道", "太平园", "高朋大道", "神仙树", "火车南站", "三瓦窑", "琉璃场", "四川师大", "狮子山", "大观", "成都东客站", "迎晖路", "槐树店", "双店路", "崔家店", "理工大学", "二仙桥", "八里庄", "府青路", "驷马桥"
      ]
    },
    {
      "service_name": "route2",
      "terminal_station": "崔家店",
      "stations": [
        "理工大学", "二仙桥", "八里庄", "府青路", "驷马桥", "火车北站", "火车北站", "北站西二路", "九里堤", "西南交大", "花照壁", "茶店子", "一品天下", "金沙博物馆", "文化宫", "东坡路", "龙爪堰", "武侯大道", "太平园", "高朋大道", "神仙树", "火车南站", "三瓦窑", "琉璃场", "四川师大", "狮子山", "大观", "成都东客站", "迎晖路", "槐树店", "双店路", "崔家店"
      ]
    }
  ]
}
```
- **station.json**：包含各站点的索引信息和换乘信息

例如成都地铁火车北站【01/03 07/01】
```json
{
  "火车北站": [
    ["01", "03"],
    ["07", "01"]
  ],
}
```
- **color.json**：包含各线路的颜色信息

配置线路主题色，例如成都地铁：
```json
{
  "line_1": "#28286A",
  "line_7": "#83C4D4",
}
```
- **trans_name.json**：包含各站的英文名称信息

例如成都地铁1号线部分站台，加上`<br>`换行：
```json
{
  "韦家碾": "Weijianian",
  "升仙湖": "Shengxian Lake",
  "火车北站": "North <br>Railway Station"
}
```
